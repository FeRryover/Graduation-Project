...
def save_edges_to_csv(edges, csv_path):
    with open(csv_path, 'w', newline='') as edge_csv_file:
        ...
        edge_csv_writer.writerow(
            ['Edge_ID', 'From_Node', 'To_Node', 'From_Latitude', 'From_Longitude', 'To_Latitude', 'To_Longitude',
             'num_lanes',
             'geometry'])
        for edge in edges:
            edge_csv_writer.writerow(
                [edge['edge_id'], edge['from_node'], edge['to_node'], edge['from_lat'], edge['from_lon'], edge['to_lat'],
